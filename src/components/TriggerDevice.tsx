...
import { Zap, WifiOff, Wifi, RefreshCw, CloudOff, Cloud } from 'lucide-react';
import { toast } from 'sonner';

const TriggerDevice: React.FC = () => {
  const { isConnected, connectionStatus, socket, reconnectSocket, offlineMode, toggleOfflineMode } = useReaction();
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-gray-900 p-4">
      ...
      <Button 
        ...
        disabled={!isConnected || isSending || offlineMode}
      ...
      <p className="mt-8 text-white/70 text-center max-w-sm">
        {offlineMode ? (
          <span className="block text-yellow-400">
            You are in offline mode. The trigger function is disabled. Switch to online mode to use it.
          </span>
        ) : !isConnected ? (
